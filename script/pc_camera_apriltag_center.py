...
class CameraAprilTag:
	def __init__(self):
		...
		rospy.logwarn("AprilTag Center PC Node [ONLINE]...")
		...
		self.cameraInfo_topic = "/pc_cv_camera/camera_info"
		...
		self.objCoord_topic = "/pc_objCoord"
...
#			rospy.loginfo("Detect ID: %d" % (result[0][1]))
	...
	def cbShowImage(self):
		...
		cv2.imshow("AprilTag Center PC", self.cv_image)
		cv2.waitKey(1)
...
#			self.cbInfo()
	...
	def cbShutdown(self):
		...
		rospy.logerr("AprilTag Center PC Node [OFFLINE]...")
		cv2.destroyAllWindows()

if __name__ == '__main__':

	# Initialize
	rospy.init_node('pc_camera_apriltag_center', anonymous=False)
